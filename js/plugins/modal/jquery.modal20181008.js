        ...
        func: {
            appendBox: function () {
                if (!H.isDefined(L.config.$objects)) {
                    ...
                    var $box = $('#w-modal-id');
                    if (0 === $box.length) {
                        $box = $('<div class="w-modal-box" id="w-modal-id"><div class="w-modal-layer"></div><div class="w-modal-body container">' +
            ...
            computeWH: function () {
                if (
                    ...
                    L.config.clientW = document.documentElement.clientWidth;
                    L.config.clientH = document.documentElement.clientHeight;
                } else {
                    L.config.clientW = window.innerWidth;
            ...
            getParams: function ($trigger) {
                ...
                switch (subOps.mode) {
                    case 'full':
                        subOps.width = L.config.clientW;
                        subOps.height = L.config.clientH;
                        break;
                    case 'auto':
                        subOps.width = L.config.clientW - 100;
                        subOps.height = L.config.clientH - 100;
                        break;
                    default :
                        subOps.width = subOps.width < L.config.clientW ? subOps.width : L.config.clientW;
                        subOps.height = subOps.height < L.config.clientH ? subOps.height : L.config.clientH;
                        break;
                }
            ...
            show: function ($trigger) { // todo
                ...
                if (subOps.title) {
                    ...
                    L.config.$objects.$content.height(subOps.height - L.config.$objects.$header.outerHeight());
                    subOps.showCloseBtn ? L.config.$objects.$closeBtn.show() : L.config.$objects.$closeBtn.hide();
                } else {
                    L.config.$objects.$header.hide();
                    L.config.$objects.$content.height(subOps.height);
                }
